...
class Config:
    ...
    # Intent Gate TRACER flags (default off = behaviour identical to upstream)
    TRACE_COLLECT = _env_flag("TRACE_COLLECT", default=False)
    INTENT_GATE = _env_flag("INTENT_GATE", default=False)
    TRACER_ARTIFACT_DIR = os.getenv("TRACER_ARTIFACT_DIR", "./tracer_data/.tracer")
    TRACE_LOG_PATH = os.getenv("TRACE_LOG_PATH", "./tracer_data/traces.jsonl")
    TRACER_EMBEDDER = os.getenv(
        "TRACER_EMBEDDER",
...
def refresh_runtime_config_from_env() -> None:
    ...
    config.TRACE_COLLECT = _env_flag("TRACE_COLLECT", default=False)
    config.INTENT_GATE = _env_flag("INTENT_GATE", default=False)
    config.TRACER_ARTIFACT_DIR = os.getenv("TRACER_ARTIFACT_DIR", "./tracer_data/.tracer")
    config.TRACE_LOG_PATH = os.getenv("TRACE_LOG_PATH", "./tracer_data/traces.jsonl")
    config.TRACER_EMBEDDER = os.getenv(
        "TRACER_EMBEDDER",
        "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2",
    )
