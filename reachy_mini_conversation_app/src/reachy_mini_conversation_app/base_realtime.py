...
if TYPE_CHECKING:
    from reachy_mini_conversation_app.intent_gate import RouteMeta, IntentGate
    from reachy_mini_conversation_app.trace_collector import TraceCollector
...
class BaseRealtimeHandler(ConversationHandler, ABC):
    ...
    def __init__(
        ...
        if config.TRACE_COLLECT:
            from reachy_mini_conversation_app.trace_collector import TraceCollector

            self.trace_collector = TraceCollector(config.TRACE_LOG_PATH)

        self.intent_gate: "IntentGate | None" = None
        if config.INTENT_GATE:
            if self.BACKEND_PROVIDER != OPENAI_BACKEND:
                logger.warning(
                    "INTENT_GATE is enabled but ignored for backend=%r (OpenAI only)",
                    self.BACKEND_PROVIDER,
                )
            else:
                try:
                    from reachy_mini_conversation_app.intent_gate import IntentGate

                    self.intent_gate = IntentGate(config.TRACER_ARTIFACT_DIR, config.TRACER_EMBEDDER)
                except Exception:
                    logger.warning("IntentGate load failed; gate disabled for this session", exc_info=True)
    ...
    async def _gate_route(self, transcript: str) -> None:
        """Route a finalized user transcript through TRACER when the intent gate is active."""
        if self.intent_gate is None:
            return
        decision, actions, meta = self.intent_gate.route(transcript)
        if decision == "bypass":
            await self._execute_bypass(transcript, actions, meta)
        else:
            await self._safe_response_create()
    ...
    async def _execute_bypass(
        ...
        """Execute bypass tools locally without triggering an LLM response."""
        had_error = False
        for tool_name, args_json in actions:
            result = await dispatch_tool_call(tool_name, args_json, self.deps)
            if isinstance(result, dict) and "error" in result:
                had_error = True
                logger.warning(
                    "IntentGate bypass tool error: %s %s -> %s",
                    tool_name,
                    args_json,
                    result["error"],
                )
            ...
            if self.connection:
                try:
                            ...
                            "content": [
                                {
                                    "type": "input_text",
                                    "text": (
                                        f"[action exécutée sans réponse vocale : {tool_name} {args_json}]"
                                    ),
                                },
                            ],
                        },
                    ...
                    logger.exception("IntentGate bypass: failed to inject conversation context")
        ...
        first_tool = actions[0][0] if actions else "unknown"
        await self.output_queue.put(
            AdditionalOutputs({"role": "assistant", "content": f"⚡ Bypass: {label} → {first_tool}"}),
        )

        if self.trace_collector is not None and actions:
            tool_raw, args_raw = actions[0]
            self.trace_collector.on_gate_bypass(
                label=label,
                accept_score=meta.accept_score,
                tool_raw=tool_raw,
                args_raw=args_raw,
                n_tools=len(actions),
            )
                ...
                async for event in self.connection:
                    ...
                    if event.type == "input_audio_buffer.speech_started":
                        ...
                        if self.trace_collector:
                            self.trace_collector.on_speech_started()
                    ...
                    if event.type == "response.done":
                        ...
                        if self.trace_collector:
                            self.trace_collector.on_response_done()
                    ...
                    if event.type == "conversation.item.input_audio_transcription.completed":
                        ...
                        if self.intent_gate is not None:
                            await self._gate_route(transcript)
                            continue
                    ...
                    if event.type == "response.function_call_arguments.done":
                        ...
                        if self.trace_collector:
                            self.trace_collector.on_tool_call(tool_name, args_json_str)
